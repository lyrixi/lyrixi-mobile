...
import type { HistoryUtilInstance, HistoryUtilNavigateParams } from './types'
...
const History = function (this: HistoryUtilInstance) {
  ...
  s.back = function () {
    // 如果未返回, 则调用返回
    setTimeout(() => {
      if (s.urlParameter && window.location.href.indexOf(s.urlParameter) !== -1) {
        window.history.go(-1)
      }
    }, 100)
  ...
  */
  s.navigate = function (urlParameter: string, params: HistoryUtilNavigateParams) {
    const { onBack } = params
...
// Static methods
for (const key in HistoryUtil) {
  ;(History as unknown as Record<string, unknown>)[key] = (HistoryUtil as unknown as Record<string, unknown>)[key]
}
