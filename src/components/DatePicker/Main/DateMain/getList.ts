...
function getList(value, type, { hourStep, minuteStep }) {
  ...
  for (let item = new Date().getFullYear() - 120; item <= new Date().getFullYear() + 120; item++) {
    years.push({
      id: item,
      name: LocaleUtil.locale(`${item}年`, 'lyrixi_41277630162b8f5811c300874c7bacac', [String(item)])
    })
  }
  ...
  for (let item = 1; item <= 12; item++) {
    months.push({
      id: item,
      name: LocaleUtil.locale(`${item}月`, 'lyrixi_0bb91f473d4f4fbdfb11f8953038e60b', [String(item)])
    })
  }
  ...
  for (let item = 0; item <= 23; item += hourStep) {
    hours.push({
      id: item,
      name: LocaleUtil.locale(`${item}时`, 'lyrixi_f093f755e17f2104b307f9f2a0f397d7', [String(item)])
    })
  }
  ...
  for (let item = 0; item <= 59; item += minuteStep) {
    minutes.push({
      id: item,
      name: LocaleUtil.locale(`${item}分`, 'lyrixi_c96aa1c9aa6ec548b71f3c4f6d6221e8', [String(item)])
    })
  }
