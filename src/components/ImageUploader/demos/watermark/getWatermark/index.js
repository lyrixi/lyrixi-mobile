import { Device, Toast, LocaleUtil } from 'lyrixi-mobile'
// 记录日志
  ...
  return new Promise(async (resolve) => {
    ...
    for (let line of watermark) {
      if (/\$distance:(\d+\.\d+,\d+\.\d+)/gim.test(line)) {
        ...
        let distanceDescription = distance === null ? '' : LocaleUtil.locale(`偏差${distance}米`)
...
async function getLocation(watermark) {
  ...
  if (
    ...
    if (locationResult?.errCode === 'PERMISSION_DENIED') {
      ...
      // 未开启定位权限阻止拍照
      Toast.show({
        content: LocaleUtil.locale('未开启定位服务或权限，定位失败')
      })
      return null
    }
  }
